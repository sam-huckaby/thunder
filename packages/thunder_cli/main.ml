...
let default_artifacts wasm runtime extra =
  let base = [ wasm; runtime ] in
  let with_assets =
    ...
    Thunder_cli_lib.Preview_publish.
      {
        ...
        artifacts = default_artifacts wasm runtime extras;
        force = force_preview ();
      }
  in
...
let () =
  ...
  | _ ->
      prerr_endline
        "Usage: thunder_cli (preview-publish|deploy-prod) [--metadata PATH] [--wasm PATH] [--runtime PATH] [--artifact PATH]";
      exit 2
